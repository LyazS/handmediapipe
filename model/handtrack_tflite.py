...
class HandTracker():
    ...
    def __init__(self,
                 ...
                 box_enlarge=1.5,
                 box_shift=0.2):
        self.palm_imsize = palm_imsize
        self.hand_imsize = hand_imsize
        ...
        self.out_idx_joint_1 = self.interp_joint.get_output_details(
        )[1]['index']
        self.out_idx_joint_2 = self.interp_joint.get_output_details(
        )[2]['index']
    ...
    def _triangle_to_bbox(source):
        ...
        bbox = np.c_[[source[2] - source[0] + source[1]],
                     [source[1] + source[0] - source[2]],
                     [3 * source[0] - source[1] - source[2]],
                     [source[2] - source[1] + source[0]], ].reshape(-1, 2)
        return bbox
    ...
    def predict_joints(self, img_norm):
        self.interp_joint.set_tensor(self.in_idx_joint, img_norm[np.newaxis])
        self.interp_joint.invoke()

        joints = self.interp_joint.get_tensor(self.out_idx_joint)
        o_1 = self.interp_joint.get_tensor(self.out_idx_joint_1)
        o_2 = self.interp_joint.get_tensor(self.out_idx_joint_2)
        return joints.reshape(-1, 3), o_1, o_2

    def detect_hand(self, img_norm):
        assert -1 <= img_norm.min() and img_norm.max() <= 1,\
        "img_norm should be in range [-1, 1]"
        assert img_norm.shape == (self.palm_imsize, self.palm_imsize, 3),\
        "img_norm shape must be ({0}, {0}, 3)".format(self.palm_imsize)
        ...
        # detecion_mask = out_clf > 0.8472978603872036
        ...
        center_wo_offst = candidate_anchors[max_idx, :2] * self.palm_imsize

        # 7 initial keypoints
        keypoints = center_wo_offst + candidate_detect[max_idx, 4:].reshape(
            -1, 2)
    ...
    def preprocess_img(self, img):
        ...
        img_pad = np.pad(img, ((pad[0], pad[0]), (pad[1], pad[1]), (0, 0)),
                         mode='constant')
        img_small = cv2.resize(img_pad, (self.palm_imsize, self.palm_imsize))
    ...
    def __call__(self, img):
        ...
        scale = max(img.shape) / self.palm_imsize
        Mtr = cv2.getAffineTransform(source * scale, self._target_triangle)

        img_landmark = cv2.warpAffine(self._im_normalize(img_pad), Mtr,
                                      (self.hand_imsize, self.hand_imsize))

        joints, o_1, o_2 = self.predict_joints(img_landmark)
        joints = joints[:, :2]
        ...
        return kp_orig, box_orig, o_1.reshape(-1)[0], o_2.reshape(-1)[0]
